...
class Parser:
    ...
    def name_parser(self, html):
        ...
        name_element = soup.find_all('h1', class_='Product__title js-datalayer-catalog-list-name')
        name_element = name_element[0].text.strip('<h1 class="Product__title js-datalayer-catalog-list-name">   ')
        return name_element
    ...
    def main_price_parser(self, html):
        ...
        price_element = soup.find_all('span', class_='js-datalayer-catalog-list-price hidden')

        price_element = price_element[0].text.strip('<h class="Product__title js-datalayer-catalog-list-name">   ')
...
def main_parser_engin():
    ...
    # каждый url проходит через парсер
